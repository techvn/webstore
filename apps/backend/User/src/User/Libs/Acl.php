...
class Acl extends ZendAcl\Acl
{
    ...
    public function __construct(UserModel $userModel)
    {
        ...
        $select->from('mds_user_acl_role')
            ->join('mds_user', 'mds_user.user_acl_role_id = mds_user_acl_role.id');
        $select->where->equalTo('mds_user.id', $this->user->getId());
    ...
    public function listResourcesByGroup($group)
    {
        $result = null;
        $select = new Select();
        $select->from(array('uar' => 'mds_user_acl_resource'))
            ->join(array('uap' => 'mds_user_acl_permission'), 'uar.id = uap.user_acl_resource_id')
